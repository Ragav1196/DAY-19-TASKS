...
// ONCLICK FUNCTION WWRITTEN HERE FOR THE PAGINATION BUTTONS
    function pageNumSelect(pageNum, pageNumStrt, pageNumEnd, i){  
        ...
        if(i >= 6 && pageNum<=100){
            if(i === 6 && pageNum<=95){
                paginationData(pageNumStrt = pageNumStrt + 1, pageNumEnd = pageNumEnd + 1, pageNum)                   
            } 
            else if(i === 7 && pageNum<=95){
                paginationData(pageNumStrt = pageNumStrt + 2, pageNumEnd = pageNumEnd + 2, pageNum)            
            }
            else if(i === 8 && pageNum<=95){
                paginationData(pageNumStrt = pageNumStrt + 3, pageNumEnd = pageNumEnd + 3, pageNum)            
            }
            else if(i === 9 && pageNum<=95){
                paginationData(pageNumStrt = pageNumStrt + 4, pageNumEnd = pageNumEnd + 4, pageNum)            
            }
            else if(i === 10 && pageNum<=95){
                paginationData(pageNumStrt = pageNumStrt + 5, pageNumEnd = pageNumEnd + 5, pageNum)            
            }
            else{
